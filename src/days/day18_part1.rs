...
impl crate::days::Day for Day18Part1 {
    fn solve(&self, input: &str) -> String {
        let instructions: Vec<_> = input.lines()
            .map(|line| line.split_once(' ').expect("line has whitespace"))
            .map(|(dir, line)| (dir, line.split_once(' ').expect("line has another whitespace")))
            .map(|(dir, (steps, _))| (Direction::parse(dir), steps.parse::<u8>().expect("steps is positive integer")))
            .collect();
...
fn complete_partition(partition: Vec<Position>, min_x: u16, max_x: u16, min_y: u16, max_y: u16, walls: &[Position]) -> (bool, Vec<Position>) {
